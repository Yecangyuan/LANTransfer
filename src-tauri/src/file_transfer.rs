...
impl FileTransferManager {
    ...
    pub async fn send_files(
        ...
        for file_path in file_paths {
            self.send_single_file(&target_device, &file_path, &app_handle).await?;
        }
        Ok(())
    }
    ...
    async fn send_single_file(
        ...
        let _ = app_handle.emit_all("transfer-progress", &progress);
    ...
    pub async fn start_file_server(&self, app_handle: tauri::AppHandle) -> Result<(), String> {
        let listener = TcpListener::bind(format!("0.0.0.0:{}", self.transfer_port)).await
            .map_err(|e| format!("Failed to bind to port {}: {}", self.transfer_port, e))?;
        
        log::info!("File transfer server listening on port {}", self.transfer_port);
            ...
            loop {
                match listener.accept().await {
                    Ok((stream, _)) => {
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = Self::handle_incoming_transfer(stream, app_handle_clone).await {
